...
- Albero ricoprente di un grafo G: $A = < N, E', \lambda_{E'} >$
- Costo di un albero ricoprente: $\sum_{e \in E'} \lambda_{E'}(e)$
- Albero minimo ricoprente: $A : \text{A è un albero ricoprente di G}\ \wedge\ \nexists\ A'\text{ albero ricoprente di G} : costo(A') < costo(A)$
